...
void printUsage() {
    std::cerr << "Usage: mdance-cli --algorithm {kmeans|divine|helm|equal}\n"
              << "                  --input <csv-path>\n"
              << "                  --output <json-path>\n"
              << "                  --natoms <int>\n"
              << "                  --nclusters <int>\n"
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (args.hasFlag("analysis")) {
            ...
            f << "  \"isim\": " << isimAll << ",\n";
            f << "  \"clusterISIM\": [";
            for (size_t i = 0; i < clusterISIM.size(); ++i) { if (i) f << ", "; f << clusterISIM[i]; }
            f << "],\n";
            f << "  \"clusterOutliers\": [";
        ...
        if (algorithm == "kmeans") {
            ...
            // Extract labels from final clusters
            result.nClusters = finalClusters.size();
            result.labels.resize(data.rows(), -1);

            for (int c = 0; c < (int)finalClusters.size(); ++c) {
                std::list<int> clusterIndices = finalClusters[c].getRootClusterIndices();
                // These are indices into the initial cluster set, need to map back to frames
                for (int initClustIdx : clusterIndices) {
                    for (int j = 0; j < initLabels.size(); ++j) {
                        if (initLabels(j) == initClustIdx) {
                            result.labels[j] = c;
                        }
                    }
                }
            ...
            throw std::runtime_error("Unknown algorithm: " + algorithm +
                ". Must be one of: kmeans, divine, helm, equal");
        }
