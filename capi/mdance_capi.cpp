    ...
    int percentage)
{
    ...
    try {
        auto data = map_coords(coords, nframes, ncols);
        ArrayXXd dataCopy = data;  // algorithms may mutate
    ...
    int nbins)
{
    ...
    try {
        auto dataMap = map_coords(coords, nframes, ncols);
        ArrayXXd data = dataMap;
        MD::Metric mt = parseMetric(metric ? metric : "MSD");
